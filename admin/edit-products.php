...
{ 
	...
	else
	{
		...
		include('connect.php');	
			...
			$query1 = " SELECT * FROM `products` where idproducts='$idproducts'";
			mysql_query("set names 'utf8'");
  ...
  echo '<form id="form1" name="form1" method="post" action="update/products.php?idproducts='.$row['idproducts'].'">';
...
echo'Tiêu đề:<br><input type="text" name="productsname"  value="'.$row['productsname'].'"><p>';
 echo 'Tóm tắt:<br><input type="text" name="productstomtat"  value="'.$row['productstomtat'].'"><p>';
  
echo 'Nội dung:<br><textarea name="productsnoidung" class="ckeditor" cols="40" rows="6">'.$row['productsnoidung'].'</textarea><p>';
echo 'Date:<br><input type="text" name="productsdate" value="'.$row['productsdate'].'"><p>';
  
echo 'Time:<br><textarea name="productstime" cols="40" rows="6">'.$row['productstime'].'</textarea><p>';

 echo 'Viewer:<br><input type="text" name="productsviewer"  value="'.$row['productsviewer'].'"><p>';
